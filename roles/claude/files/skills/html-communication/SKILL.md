...
description: Publish a report, write-up, plan, spec, or findings as a hosted HTML document. Use when the user asks for a report as HTML, or mentions "HTML" with no other context.
metadata:
...
Reports only: a plan, spec, write-up, findings, summary, or comparison the user will read.
UI mocks and interactive pages go through the built-in Artifact tool instead. HTML that
ships as part of a product is not this skill.
...
Jeremy has given standing permission to upload every document created or updated with this skill. Upload is required, including in Auto mode. Do not ask for separate permission or stop at the local file.
